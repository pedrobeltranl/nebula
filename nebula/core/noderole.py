...
""")
    ...
    async def extended_learning_cycle(self):
        ...
        # FIX: Logic to handle lock acquisition and training safely without returning early on failure
        ...
        try:
            ...
            logging.error(f"[Honeypot] ⚠️ Injection/Training Warning: {e}. Continuation with unpoisoned model.")
            # Ensure LR is restored if something went wrong before the inner finally
            try:
                 self._engine.trainer.update_model_learning_rate(original_lr)
            except:
                 pass
        ...
        try:
            ...
            # Publish Update
            logging.info(f"[Honeypot] Publishing update (Poisoned: {injection_successful})...")
            self_update_event = UpdateReceivedEvent(
                self._engine.trainer.get_model_parameters(), self._engine.trainer.get_model_weight(), self._engine.addr, self._engine.round
            )
            await EventManager.get_instance().publish_node_event(self_update_event)
        ...
        # ---------------------------------------------------------
        # PHASE 5: HONEYPOT ANALYSIS (Detector)
        # Verify if neighbors reacted to the HoneyDoor
        # ---------------------------------------------------------
        try:
            logging.info("[Honeypot] Analyzing neighbor responses...")

            # Access raw updates from the Aggregator's storage
            if hasattr(self._engine, "aggregator") and hasattr(self._engine.aggregator, "us"):
                updates_storage = self._engine.aggregator.us.us # Dict[source, (latest_update, history)]
                ...
                # Use a temporary model for verification
                temp_model = copy.deepcopy(self._engine.trainer.model)
                ...
                try:
                    ...
                    logging.warning(f"[Honeypot] Could not get validation data for detector: {data_e}")
                    val_data = None

                current_round = self._engine.round

                if val_data:
                    for source, (latest_update, history) in updates_storage.items():
                        # Only check updates from THIS round (or very recent) - allow slight async slip
                        if latest_update.round >= current_round - 1:

                            # Skip self
                            if source == self._engine.addr:
                                continue

                            # Load neighbor's parameters into temp_model
                            try:
                                temp_model.load_state_dict(latest_update.model)

                                # Run Detection
                                is_threat = self.detector.check(
                                    temp_model,
                                    val_data,
                                    self.manager.honeymap_rules
                                )

                                if is_threat:
                                    logging.warning(f"[Honeypot] 🚨 THREAT DETECTED: Node {source} triggered HoneyMap!")
                                    self.honeymap_confirmed_threats.add(source)

                                    # Report to Reputation System
                                    if hasattr(self._engine, "_reputation"):
                                        self._engine._reputation.update_reputation(source, -0.5)

                            except Exception as check_e:
                                logging.error(f"[Honeypot] Failed to check neighbor {source}: {check_e}")

        except Exception as e:
            logging.error(f"[Honeypot] ⚠️ Analysis Phase Error: {e}")
